class MorseAudioContext {
  constructor() {
    ...
    if (this.AudioContext === undefined) {
      ...
      return; // throw (new Error("No AudioContext class defined"));
    }
    ...
    this.isIOS = ua.indexOf("iphone") >= 0 && ua.indexOf("like iphone") < 0 || ua.indexOf("ipad") >= 0 && ua.indexOf("like ipad") < 0 || ua.indexOf("ipod") >= 0 && ua.indexOf("like ipod") < 0; // if (this.isIOS) this.playHTMLaudio();
  }
  /**
  ...
  getAudioContext() {
    ...
    if (this.audioContext !== undefined) {
      if (this.audioContext.state === "running") {// console.log("AC is running");
      } else {
        // console.log("AudioContext is suspended");
        this.audioContext.resume().then(() => {
          this.runUnlockedActions(1);
        });
      }
    } else {
      ...
      console.log(`AudioContext state: ${this.audioContext.state}`); // Will only work if using Firefox (and will take a short while) or where this method is called the first time with a user interaction, otherwise will be ignored

      this.audioContext.resume().then(() => {
        this.runUnlockedActions(2);
      });
    }
  ...
  decodeSample(key) {
    ...
    console.log(`Decoding audio file (${key})`); // Promise-based syntax does not work for Safari desktop, need to use callback variant
