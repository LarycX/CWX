...
export default class MorsePlayerWAA {
  ...
  set frequency(freq) {
    ...
    } catch (e) {// getting here means _initialiseAudio() has not yet been called: that's okay
    }
  }
  ...
  set playMode(mode) {
    ...
    this._playMode = mode; // force re-evaluation of volume and frequency in case the mode has been changed during playback
  ...
  set volume(v) {
    ...
    try {
      let now = morseAudioContext.getAudioContext().currentTime; // change volume linearly over 30ms to avoid discontinuities and resultant popping
  ...
  load(sequence) {
    ...
    let frequencies = sequence.frequencies || this.fallbackFrequency; // TODO: add volume array
  ...
  play() {
    ...
    this._tZero = morseAudioContext.getAudioContext().currentTime - this._cTimings[this._nextNote] + Math.max(this.startPadding, this._initialStartPadding) / 1000;
  ...
  pause() {
    ...
    if (!this.isNote[this._nextNote]) {
      this._nextNote++; // if we've got to the end of the sequence, then loop or load next sequence as appropriate
  ...
  _scheduleNotes() {
    ...
    while (this._nextNote < this.sequenceLength && this._cTimings[this._nextNote] < nowAbsolute - this._tZero + this._lookAheadTime) {
      // this._notPlayedANote = false;
      var nowRelative = nowAbsolute - this._tZero; // console.log('T: ' + Math.round(1000 * nowAbsolute)/1000 + ' (+' + Math.round(1000 * nowRelative)/1000 + ')');
      ...
      if (this.isNote[this._nextNote]) {
        ...
        if (this._playMode === 'sine') {
          ...
          let sounds = morseAudioContext.getSounds(); // start and stop the "on" sound
          ...
          try {
            ...
            if (stop) {
              bsn.stop(stop);
            } // if we don't schedule a stop then the sound file plays until it completes
          ...
          try {
            ...
            if (stop2) {
              bsn.stop(stop2);
            } // we won't have the stop time for the final off sound, so just let it run
