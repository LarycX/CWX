...
function initRuntimeSocketService() {
  const hosts = "127.0.0.1,192.168.8.198,198.18.0.1";
  const port = "8090";
  const id = "mp-weixin_cybFab";
...
var dictionary$4 = {
  ...
  letter: {
    ...
    ")": ". . . . .s. .s. .",
    '"': ". . - .s- .",
    // fall back to using open quotes for straight
    "“": ". . - .s- .",
    // open quotes
    "”": ". . - .s- . - .",
    // close quotes
...
var dictionaries = {};
dictionaries.international = dictionary$5;
dictionaries.american = dictionary$4;
dictionaries.cyrillic = dictionary$3;
dictionaries.cyrillicUkraine = dictionary$2;
dictionaries.hebrew = dictionary$1;
...
class Morse {
   ...
   */
  constructor({
    dictionary: dictionary2 = "international",
    dictionaryOptions = []
  } = {}) {
    if (typeof dictionary2 === "string") {
      dictionary2 = [dictionary2];
    }
    this.setDictionariesAndOptions(dictionary2, dictionaryOptions);
  }
  ...
  _loadDictionaries() {
    ...
    for (let optName of this.options) {
      if (this.dictionary.options[optName] !== void 0) {
   ...
   */
  display(tokens, charSpace, wordSpace, map = {}, errors = void 0, errorPrefix = "", errorSuffix = "") {
    for (let k in map) {
  ...
  displayMorse(morseTokens) {
    return this.display(morseTokens, this.dictionary.display.join.charSpace, this.dictionary.display.join.wordSpace, this.dictionary.display.morse);
  }
  displayMorseErrors(morseTokens, errorTokens, prefix, suffix) {
    return this.display(morseTokens, this.dictionary.display.join.charSpace, this.dictionary.display.join.wordSpace, this.dictionary.display.morse, errorTokens, prefix, suffix);
  }
...
class MorseCW extends Morse {
  ...
  testFWPMmatchesRatio() {
    return Math.abs(this.ratios["wordSpace"] / this.dictionary.ratio["wordSpace"] / (this.ratios["charSpace"] / this.dictionary.ratio["charSpace"]) - 1) < 1e-3;
  }
  /** @type {number[]} */
  get ratios() {
    if (this._ratios === void 0) {
      this._ratios = {};
  ...
  setFWPMfromRatio(ratio) {
    ratio = Math.max(Math.abs(ratio), 1);
...
Email: steve@scphillips.com
*/
class MorseCWWave extends MorseCW {
  /**
   * @param {Object} params - dictionary of optional parameters.
  ...
  getSample(timings, endPadding = 0) {
    ...
    var y0, y1 = 0, y2 = 0;
    var gain = 0.813;
    for (var t2 = 0; t2 < timings.length; t2 += 1) {
      duration = this.sampleRate * Math.abs(timings[t2]) / 1e3;
      on = timings[t2] > 0 ? 1 : 0;
      for (var i = 0; i < duration; i += 1) {
        x0 = on * Math.sin(i * step);
  ...
  getWAASample(timings, endPadding = 0) {
    ...
    if (offlineAudioContextClass === void 0) {
      throw new Error("No OfflineAudioContext class defined");
    }
    var offlineCtx = new offlineAudioContextClass(1, this.sampleRate * (this.getDuration() + endPadding) / 1e3, this.sampleRate);
    var gainNode = offlineCtx.createGain();
    gainNode.gain.setValueAtTime(0.813, 0);
    var lowPassNode = offlineCtx.createBiquadFilter();
    lowPassNode.type = "lowpass";
    lowPassNode.frequency.setValueAtTime(this.frequency * 1.1, 0);
    ...
    for (var i = 0; i < timings.length; i++) {
      duration = Math.abs(timings[i]) / 1e3;
      if (timings[i] > 0) {
        oscillator = offlineCtx.createOscillator();
        oscillator.type = "sine";
        oscillator.frequency.setValueAtTime(this.frequency, t2);
        oscillator.start(t2);
        oscillator.stop(t2 + duration);
...
exports.MorseCWWave = MorseCWWave;
